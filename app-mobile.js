...
function initAuthGate(onUnlock) {
  ...
  const gate2 = document.getElementById('authGate2');
  const app = document.querySelector('.app');
  if (!gate) { app?.classList.add('ready'); onUnlock?.(); return; }
  ...
  if (isAuthedL1()) {
    ...
    if (gate2) {
      gate2.style.display = 'flex';
      startAuthFloat('authParticlesL2');
      bindLevel2Auth(onUnlock);
      setTimeout(() => document.getElementById('authPwd2')?.focus(), 200);
    }
    return;
  }
  ...
  const submitL1 = () => {
    const v = (inp?.value || '').trim();
    if (!v) {
      if (msg) { msg.className = 'auth-msg'; msg.textContent = '要先输入小口令哦～'; }
      return;
    }
    if (AUTH_PASS_L1.includes(v)) {
      try { sessionStorage.setItem(AUTH_KEY_L1, '1'); } catch {}
      const rect = btn?.getBoundingClientRect();
      if (rect) spawnAuthBurst(rect.left + rect.width / 2, rect.top + rect.height / 2);
      bindLevel2Auth(onUnlock);
      showLevel2Gate();
      return;
    }
    ...
    if (msg) {
      ...
      msg.textContent = fails === 1 ? '入口已上锁，答对才能进～' : '哎呀，口令不对哦～只有婉宁小朋友可以进入的✨';
    }
    if (inp) { inp.value = ''; inp.focus(); }
  };
  ...
  inp?.addEventListener('keydown', e => { if (e.key === 'Enter') submitL1(); });
  inp?.addEventListener('focus', e => {
    const r = e.target.getBoundingClientRect();
    spawnAuthBurst(r.left + 40, r.top);
  });
